...
type persistedAccount struct {
	...
	// NtfyEnabled is carried for migration only; the per-phone NtfyStore is
	// the source of truth once it has an entry.
		...
		for _, l := range legacy {
			if !seen[l.Phone] {
				...
				state.Accounts = append(state.Accounts, persistedAccount{
					...
					InstanceID: l.InstanceID, ExpiresAt: l.ExpiresAt, NtfyEnabled: l.NtfyEnabled,
				})
			}
...
func (sm *SessionManager) RestoreSessions(store *SessionStore, logger *slog.Logger) int {
	...
	for _, entry := range state.Accounts {
		...
		acct := sm.RestoreAccount(entry.Phone, auth, logger)
		restored++
		sm.logger.Info("Restored account", "phone", entry.Phone)

		// Migrate a legacy ntfy flag into the per-phone store (source of truth,
		// already consulted by getOrCreateAccount) so future restarts pick it up.
		if entry.NtfyEnabled {
			if sm.ntfyStore != nil && !sm.ntfyStore.Load(entry.Phone).Enabled {
...
func (sm *SessionManager) persistSessions(store *SessionStore) {
	...
	for phone, acct := range sm.accounts {
		...
		state.Accounts = append(state.Accounts, persistedAccount{
			...
			InstanceID: a.InstanceID, ExpiresAt: a.ExpiresAt, NtfyEnabled: acct.NtfyEnabled,
		})
	}
